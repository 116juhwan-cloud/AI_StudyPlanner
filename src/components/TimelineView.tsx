...
import { Search, Bell, ChevronLeft, ChevronRight, Calendar, Award, Trash2, Plus, Play, Timer, X, CheckCircle2, Pause } from 'lucide-react';
...
export default function TimelineView({ tasks, onDeleteTask, onNavigateToAddTask }: TimelineViewProps) {
  ...
  // 실시간으로 시작 시간이 된 일정 감시 (5초 간격)
  useEffect(() => {
    const checkTime = setInterval(() => {
      const now = new Date();
      const currentHHMM = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
      const todayYMD = now.toLocaleDateString('en-CA'); // YYYY-MM-DD format
        ...
        <div className="fixed top-6 right-6 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="bg-surface-container-high border-2 border-primary/20 rounded-2xl p-5 shadow-2xl flex items-start gap-4 max-w-sm backdrop-blur-md">
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-on-surface-variant mb-4 font-medium leading-tight">계획하신 학습 시간이 되었습니다. 지금 바로 시작하시겠습니까?</p>
              <div className="flex gap-2">
                <button 
                  onClick={() => {
                    if (activeTaskId) {
                      setTaskAccumulatedSeconds(prev => ({ ...prev, [activeTaskId]: elapsedSeconds }));
                    }
                  ...
                  className="flex-1 bg-primary text-on-primary py-2 rounded-lg text-xs font-bold hover:bg-primary/90 transition-all flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                >
                        ...
                        {(isActive || (taskAccumulatedSeconds[t.id] > 0 && !isCompleted)) && (
                          <div className={`absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[10px] font-bold shadow-lg flex items-center gap-1.5 transition-all ${
                            ...
                            <div className="relative flex h-2 w-2">
                              <span className={`${isActive ? 'animate-ping' : ''} absolute inline-flex h-full w-full rounded-full ${isActive ? 'bg-on-primary' : 'bg-amber-500'} opacity-75`}></span>
                              <span className={`relative inline-flex rounded-full h-2 w-2 ${isActive ? 'bg-on-primary' : 'bg-amber-600'}`}></span>
                            </div>
                        ...
                        <p className="text-xs text-on-surface-variant mt-2 leading-relaxed">
                         ...
                         <div className="flex gap-1.5 flex-wrap items-center mt-3">
                          ...
                          {(taskAccumulatedSeconds[t.id] > 0 || isActive) && (
                            <span className="text-[10px] text-primary font-bold whitespace-nowrap bg-primary/5 px-1.5 py-0.5 rounded border border-primary/10">
                          ...
                          {!isActive && !isCompleted && (
                            <button 
                              ...
                              className="ml-auto text-[10px] bg-primary text-on-primary px-2.5 py-1 rounded font-bold flex items-center gap-1 hover:opacity-90 transition-all cursor-pointer"
                            >
                          ...
                          {isActive && (
                            <div className="ml-auto flex gap-1.5">
                              <button 
                                ...
                                className="text-[10px] bg-amber-500 text-white px-2.5 py-1 rounded font-bold flex items-center gap-1 hover:opacity-90 transition-all cursor-pointer"
                              >
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  if (elapsedSeconds < requiredSeconds) {
                                    alert(`아직 목표 학습 시간(${t.estimatedTime}분)을 채우지 못했습니다. 조금 더 힘내세요!`);
                                    return;
                                  }
                                  setTaskAccumulatedSeconds(prev => ({ ...prev, [t.id]: elapsedSeconds }));
                                ...
                                className={`text-[10px] px-2.5 py-1 rounded font-bold flex items-center gap-1 transition-all cursor-pointer ${
                                  elapsedSeconds >= (t.estimatedTime || 0) * 60
                                    ? 'bg-green-600 text-white hover:opacity-90 shadow-md'
                          ...
                          {isCompleted && !isActive && (
                            <button 
                              ...
                              className="ml-auto text-[10px] bg-surface-container-high text-on-surface-variant px-2.5 py-1 rounded font-bold flex items-center gap-1 hover:bg-surface-container-highest transition-all cursor-pointer"
                            >
                              <Play className="w-2.5 h-2.5 fill-current" /> 다시 시작
                            </button>
                          )}
                        </div>
                      </div>
